...
type ReportContext struct {
	...
	StartDate     xsdDate     `xml:"date"`
	EndDate       xsdDate     `xml:"dateEnd"`
	Month         string      `xml:"month"`
	Year          string      `xml:"year"`
	ReportVersion float64     `xml:"reportVersion"`
	ReportStatus  string      `xml:"reportStatus"`
	Installation  Installation
	Operator      RoleName `xml:"operator"`
	IssueDate     xsdDate  `xml:"issueDate"`
	ApprovalDate  xsdDate  `xml:"approvalDate"`
	Approver      RoleName `xml:"approver"`
}
...
type Welltest struct {
	...
	LastValidTest     xsdDateTime        `xml:"lastValidTest"`
	TestReason        string             `xml:"testReason"`
	TestType          string             `xml:"testType"`
	ProductionTest    ProductionWellTest `xml:"productionTest"`
	InjectionWelltest InjectionWellTest  `xml:"injectionTest"`
}

type ProductionWellTest struct {
	TestDuration         DoubleValue          `xml:"testDuration"`
	WellHeadData         WellHeadDataWellTest `xml:"wellheadData"`
	SeparatorData        SeparatorData        `xml:"separatorData"`
	ProductionTestResult ProductionTestResult `xml:"productionTestResults"`
}

type InjectionWellTest struct {
	TestDuration            DoubleValue         `xml:"testDuration"`
	InjectedFluid           string              `xml:"InjectedFluid"`
	ChokeRelative           DoubleValue         `xml:"chokeRelative"`
	WellheadFlowingPressure DoubleValue         `xml:"wellheadFlowingPressure"`
	InjectionTestResult     InjectionTestResult `xml:"injectionTestResults"`
}

type WellHeadDataWellTest struct {
	Temperature             DoubleValue `xml:"temperature"`
	FlowingPressure         DoubleValue `xml:"flowingPressure"`
	ChokeSize               DoubleValue `xml:"chokeOrificeSize"`
	ChokeRelative           DoubleValue `xml:"chokeRelative"`
	GasLiftRate             DoubleValue `xml:"gasLiftRate>fluidRateStdTempPres"`
	GasLiftChokeSize        DoubleValue `xml:"gasLiftchokeOrificeSize"`
	WellheadFlowingPressure DoubleValue `xml:"wellheadFlowingPressure"`
}

type SeparatorData struct {
	SeparatorPressure    DoubleValue `xml:"separatorPressur"`
	SeparatorTemperature DoubleValue `xml:"separatorTemperature"`
}

type InjectionTestResult struct {
	WaterRateStdTemPressure DoubleValue `xml:"waterRate>fluidRate"`
}

type ProductionTestResult struct {
	OilRateStdTemPressure   DoubleValue `xml:"oilRate>fluidRateStdTempPres"`
	GasRateStdTemPressure   DoubleValue `xml:"gasRate>fluidRateStdTempPres"`
	WaterRateStdTemPressure DoubleValue `xml:"waterRate>fluidRateStdTempPres"`
	GoR                     DoubleValue `xml:"gasOilRatio"`
}
...
type ProdVolume struct {
	...
	StartDate          xsdDate              `xml:"dateStart"`
	EndDate            xsdDate              `xml:"dateEnd"`
	StartTime          xsdDateTime          `xml:"dTimStart"`
	EndTime            xsdDateTime          `xml:"dTimEnd"`
...
type Parameter struct {
	StartDate    xsdDate `xml:"dateStart"`
	EndDate      xsdDate `xml:"dateEnd"`
	MeasureValue Value   `xml:"measureValue"`
}

func (pSet *ParameterSet) GetParameterDataWithName(name string) Parameter {
	for i := 0; i < len(pSet.Parameters); i++ {
		if strings.ToLower(pSet.Name) == strings.ToLower(name) {
			return pSet.Parameters[i]
